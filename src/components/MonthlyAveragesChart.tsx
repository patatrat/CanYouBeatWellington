...
// recharts clones this element at render time, injecting x/y/payload/index —
// the props are only ever undefined when TS evaluates the JSX below.
const SeasonTick = ({ x = 0, y = 0, payload = { value: "" }, index = 0 }: SeasonTickProps) => (
  <g transform={`translate(${x},${y})`}>
    <text x={0} y={0} dy={12} textAnchor="middle" fontSize={11} fill="#9ca3af">
      {payload.value}
    </text>
    <rect x={-10} y={17} width={20} height={6} rx={2} fill={MONTH_SEASON_COLORS[index]} opacity={0.8} />
  </g>
);

const CustomTooltip = ({ active, payload, label }: { active?: boolean; payload?: { payload: MonthDatum }[]; label?: string }) => {
...
const MonthlyAveragesChart = ({ history }: MonthlyAveragesChartProps) => {
  ...
  return (
    <ResponsiveContainer width="100%" height={220}>
      <BarChart data={data} margin={{ top: 4, right: 4, bottom: 4, left: -20 }}>
        ...
        <XAxis dataKey="month" tick={<SeasonTick />} axisLine={false} tickLine={false} height={38} />
        <YAxis
