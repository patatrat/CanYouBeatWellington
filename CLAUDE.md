...
- [x] **Special dates system** (2026-06-28) — went well beyond the original "overlay a quip" sketch. New `special_date_defs`/`special_date_occurrences` Neon tables support fixed-rule annual holidays (small `nth_weekday`/`fixed` rule DSL, auto-regenerated yearly via the daily cron), manually-curated moveable festivals (Cuba Dupa, WOW — entered by hand each year, never auto-generated), and sporting events with a semi-automated pending→confirmed resolution (`POST /api/admin/resolve-sporting`, same `Bearer` pattern as `CRON_SECRET`, needs a new `ADMIN_SECRET`). Each occasion can override the quip, background (`src/utils/specialBackgrounds.ts`), trigger a confetti/balloon effect (`SpecialDateEffect.tsx`), link to an event page, and — the headline mechanic — flip the entire weather verdict ("a Wellington team winning forces a good day regardless of weather"). Calendar shows a sparkle badge. Data layer: `src/lib/special-dates.ts`, 21 tests. Also extracted `src/lib/ap-posting.ts` (was duplicated between the cron and `scripts/announce.js`) and fixed `announce.js`'s import, broken since the migration. See `src/lib/__tests__/special-dates.test.ts` for the precedence/override rules.
- [x] **Seed real special dates** (2026-06-28) — 17 defs / 30 occurrences (2026–2027) live in Neon. Fixed-rule (auto-regenerate yearly via the cron): Wellington Anniversary Day, Waitangi Day, Good Friday, Easter Monday (added an `easter_offset:N` rule type to `computeFixedRuleDate` to support these — Easter isn't a fixed date or nth-weekday), ANZAC Day, King's Birthday, Labour Day, New Year's Day, Day after New Year's Day, Christmas Day, Boxing Day. Moveable (manually dated, re-entered each year): Matariki (2026-07-10, 2027-06-25), CubaDupa, WOW, Beervana — all with real looked-up dates and links. Sporting: the Hurricanes' actual 2026 Super Rugby Pacific Grand Final win (seeded retroactively as `confirmed`/`won`) and the All Blacks v Italy test at Sky Stadium on 2026-07-11 (seeded as `pending` — the live test of the semi-automated `/api/admin/resolve-sporting` workflow). Wellington Sevens dropped — no longer a real event per the user. `specialBackgrounds.ts`'s unused `sevens` key replaced with `rugby` (Hurricanes/All Blacks) and `festive` (Christmas/New Year's).
