...
- [ ] **Seed real special dates** — the system above ships with an empty table; explicitly deferred. Need to add real `special_date_defs`/occurrences for Wellington Anniversary Day, Waitangi Day, ANZAC Day, Matariki, Wellington Sevens, and any moveable festivals (Cuba Dupa, WOW) worth covering.
- [ ] **Add `ADMIN_SECRET` to Vercel + GitHub secrets** — required for `/api/admin/resolve-sporting` to work in production; same process as `CRON_SECRET` was added.
