...
- [ ] **Add test coverage for `src/lib/weather.ts`** — `weatherStorage.js`/`weatherStorage.test.js` no longer exist post-migration (client-side fetch + localStorage cache was replaced by server-side Neon queries); `src/lib/weather.ts` has zero test coverage currently (only `rulesStorage.test.ts` exists). Worth covering: `fetchLiveWeather()`'s response-shape guard, the missing equivalent guard in `fetchAndStoreBatch()` (see the P2 bug fix item above), and `getTodaysNZTDate()`'s timezone handling. Would need to mock `fetch` (Open-Meteo) and the Neon `sql` client.
