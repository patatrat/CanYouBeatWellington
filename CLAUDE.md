...
| `src/app/api/admin/refresh-note/route.ts` | Re-stamps a previously-published note with the current `interactionPolicy`/etc. shape and sends a signed `Update` to followers — for backfilling posts published before some later field existed, `ADMIN_SECRET` bearer auth |
